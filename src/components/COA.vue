<template>
    <q-dialog v-model="dlgCOA" persistent transition-show="scale" transition-hide="scale">
      <q-card class="icard" bordered style="width: 800px; max-width: 80vw; max-height: 700px">
        ...
        <q-table square :rows="data" :columns="columns" no-data-label="data kosong"
          ...
          selection="single" virtual-scroll dense class="fit" table-style="min-height:302px; max-height: 450px"
              ...
              <q-td v-for="col in props.cols" :key="col.name" :props="props">
                <div class="grid-data">
                  <div v-if="col.name === 'account_no'">
                    <q-btn :label="props.row.account_no" no-caps dense flat @click="selectdata(props.row.account_no)" />
                  </div>
                  <div v-else>
                    {{ col.value }}
                  </div>
