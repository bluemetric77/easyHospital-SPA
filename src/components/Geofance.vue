<template>
    <q-dialog v-model="dlgGeofance" persistent transition-show="scale" transition-hide="scale">
      <q-card square class="icard" style="min-width: 500px; max-width: 80vw; max-height: 700px">
        ...
        <q-table square :rows="data" :columns="columns" no-data-label="data kosong"
          no-results-label="data yang kamu cari tidak ditemukan" row-key="geofance_name" :filter="filter"
          separator="cell" selection="single" virtual-scroll dense class="fit"
          table-style="min-height:302px; max-height: 450px" v-model:selected="selected" v-model:pagination="pagination"
          binary-state-sort @request="onRequest" :loading="loading">
              ...
              <q-td v-for="col in props.cols" :key="col.name" :props="props">
                <div class="grid-data">
                  <div v-if="col.name === 'partner_id'">
                    <q-btn :label="props.row.partner_id" no-caps dense flat @click="selectdata(props.row.partner_id)" />
                  </div>
                  <div v-else>
                    {{ col.value }}
                  </div>
