<template>
    <q-dialog v-model="dlgPartner" persistent transition-show="scale" transition-hide="scale" full-width>
      <q-card square class="icard" style="width: 800px; max-width: 80vw; max-height: 700px">
              ...
              <q-td v-for="col in props.cols" :key="col.name" :props="props">
                <div class="grid-data">
                  <div v-if="col.name === 'partner_id'">
                    <q-btn :label="props.row.partner_id" no-caps dense glossy @click="selectdata(props.row.partner_id)" class="bg-green-10 text-white"/>
                  </div>
                  <div v-else>
                    {{ col.value }}
                  </div>
