              ...
              <q-td v-for="col in props.cols" :key="col.name" :props="props">
                <div class="grid-data">
                  <div v-if="col.name === 'vehicle_no'">
                    <q-btn :label="props.row.vehicle_no"  no-caps @click="selectdata(props.row.vehicle_no)"
                      class="bg-green-10 text-white" glossy/>
                  </div>
                  <div v-else>
                    {{ col.value }}
                  </div>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    function selectdata(vehicle_no='') {
      if ((selected.value.length > 0) || (vehicle_no !=='')){
        let row=null
        if (vehicle_no !==''){
           data.value.forEach(el=>{
             if (vehicle_no === el.vehicle_no) {
              row =el
            }
           }) 
        } else {
    ...
    async function onRequest(props) {
      ...
      try {
        let prop = {
          ...
          url: "master/operational/vehicle/open",
        };
    ...
    function closedata(record) {
      ...
      filter.value = "";
      data.value = [];
      context.emit("CloseVehicle", false, record);
    }
