  ...
  <q-layout view="hHh Lpr lFf">
    ...
    <q-drawer v-model="drawer" :mini="miniState" mini-to-overlay @click.capture="drawerClick"
      ...
      <q-scroll-area :thumb-style="thumbStyle" :bar-style="barStyle" style="
        ...
        ">
        <q-list dense v-for="(items, index) in detail(-1, 0)" :key="index" class="text-white" transition="scale">
          <q-expansion-item dense-toggle :icon="items.icons" :label="items.title" header-class="bg-teal-9 text-white"
            :content-inset-level="0.2" class="shadow-5 overflow-hidden q-mb-sm"
            ...
            <q-list padding dense>
              <div v-for="(itmdtl, idxdtl) in detail(items.sort_number, 1)" :key="idxdtl">
                ...
                <div v-else-if="(itmdtl.is_parent === false)">
                  <q-item clickable v-ripple :active="link === itmdtl.id" @click="openlink(itmdtl.url_link, itmdtl.id)"
                ...
                <div v-else>
                  <q-expansion-item dense-toggle expand-separator :icon="itmdtl.icons" :label="itmdtl.title"
                    header-class="text-green" :header-inset-level="0.2" :content-inset-level="0.4">
                    <q-list v-for="(itmsub, idxsub) in detail(itmdtl.sort_number, 2)" :key="idxsub">
                      <div v-if="itmsub.title === 'separator'">
                        <q-separator spaced inset />
                      </div>
                      <div v-else-if="itmsub.is_header===true">
                        <q-item clickable v-ripple :active="link === itmdtl.id" @click="openlink(itmdtl.url_link, itmdtl.id)"
                          active-class="my-menu-link" header-class="text-white">
                          <q-item-section avatar>
                            <q-icon :name="itmdtl.icons" size="xs" />
                      ...
                      <div v-else>
                        <q-item clickable v-ripple :active="link === itmsub.id"
                          @click="openlink(itmsub.url_link, itmsub.id)" active-class="my-menu-link">
                          <q-item-section avatar>
