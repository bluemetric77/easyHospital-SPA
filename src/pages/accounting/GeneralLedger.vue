  ...
  <q-page class="page-app">
    <q-card v-show="stateform" square class="icard">
      <q-bar dense class="entry-caption">
        ...
        <q-space />
        <q-btn label="Kembali" flat dense no-caps size="sm" icon="mdi-arrow-collapse-left" @click="stateform=false" />
      </q-bar>
      <q-card-section class="q-pa-sm">
        <div class="row items-start q-col-gutter-xs q-mb-sm">
          <div class="col-xs-12 col-sm-6 col-md-6">
            <div class="row items-start q-mb-sm q-col-gutter-xs">
              <div class="col-xs-8 col-sm-8 col-md-5">
                <q-select v-model="edit.trans_code" :options="voucher" outlined dense options-dense label="Voucher"
                  option-value="voucher_code" option-label="descriptions" emit-value map-options fill-input square />
              </div>
              <div class="col-xs-4 col-sm-4 col-md-2">
                <q-input v-model="edit.trans_series" dense outlined readonly type="text" label="Series" square />
              </div>
              <div class="col-xs-6 col-sm-6 col-md-3">
                <q-input v-model="edit.ref_date" dense outlined type="date" label="Tanggal" square />
              </div>
            </div>
            <div class="row items-start q-col-gutter-xs q-mb-sm">
            ...
            <div class="row items-start q-col-gutter-xs q-mb-sm">
              <div class="col-xs-12 col-sm-12 col-md-10">
                <q-input v-model="edit.notes" dense outlined type="text" label="Catatan" square />
              </div>
            </div>
          </div>
          <div class="col-xs-12 offset-sm-1 col-sm-5 offset-md-2 col-md-4">
            <div class="row items-start q-col-gutter-xs q-mb-sm">
              <div class="col-6">
                <q-field v-model="edit.debit" dense outlined label="Debit" square>
              ...
              <div class="col-6">
                <q-field v-model="edit.credit" dense outlined label="Kredit" square>
                  <template v-slot:control="{ id, value, emitValue }">
                    <vue-numeric :id="id" precision="2" :value="value" thousand-separator="."
                      class="q-field__input text-right" read-only @input="emitValue" />
                  </template>
            ...
            <div class="row items-start q-mb-sm q-col-gutter-xs q-mb-sm">
              <div class="col-12">
                <q-select v-model="edit.transtype" :options="Jurnaltypes" outlined dense options-dense
      ...
      <q-card-actions v-show="isvoid">
        <q-btn label="Void Jurnal" no-caps class="glossy" color="red" icon="remove_circle_outline" />
      </q-card-actions>
    </q-card>
    <q-card v-show="!stateform" square class="q-mb-sm icard">
      <q-bar dense class="entry-caption">
      ...
      <q-card-section class="q-pa-xs">
        <div class="row items-center q-mb-sm q-col-gutter-sm">
          <div class="col-xs-12 col-sm-3 col-md-2">
            <q-input v-model="no_account" outlined dense label="No.Akun" square>
              <template v-slot:append>
                <q-icon name="search" @click="openaccount()" />
              </template>
            </q-input>
          </div>
          <div class="col-xs-12 col-sm-5 col-md-4">
            <q-input v-model="nm_account" outlined dense label="Nama Akun" disable square />
          </div>
        </div>
        <div class="row items-center q-col-gutter-xs q-mb-sm">
          <div class="col-xs-12 col-sm-5 col-md-4">
            <q-select v-model="project_code" :options="projects" outlined dense options-dense label="Proyek Akunting"
              option-value="project_code" option-label="project_name" emit-value map-options fill-input square />
          </div>
        </div>
        <div class="row items-center q-mb-sm q-col-gutter-sm">
          <div class="col-xs-4 col-sm-3 col-md-2">
            <q-input v-model="date1" outlined dense label="Tanggal" square type="date" />
          </div>
          <div class="col-xs-4 col-sm-3 col-md-2">
            <q-input v-model="date2" type="date" dense label="s/d" outlined square />
          </div>
          <div class="col-xs-4 col-sm-3 col-md-2">
            <q-btn icon="refresh" label="Query" class="glossy" no-caps color="teal-8" size="md" @click="loaddata()" />
          </div>
        </div>
      </q-card-section>
    </q-card>
    <q-card v-show="!stateform" square class="icard q-mb-sm">
      ...
      <q-table class="grid-tables" square :data="data" :columns="columns" dense no-data-label="data kosong"
        no-results-label="data yang kamu cari tidak ditemukan" row-key="sysid" separator="cell" selection="single"
        v-model:selected="selected" v-model:pagination="pagination" binary-state-sort @request="onRequest">
            ...
            <q-td v-for="col in props.cols" :key="col.name" :props="props">
              <div class="grid-data">
                <div v-if="col.name==='action'">
                  <q-icon v-for="(btn, index) in btns" v-show="!(btn.action==='refresh') && !(btn.action==='add')"
                    :key="index" flat no-caps dense class="q-mr-xs" :name="btn.icon" size="xs" color="teal-8"
                ...
                <div v-else-if="col.name==='is_void'">
                  <q-toggle v-model="props.row.is_void" checked-icon="check" color="green" unchecked-icon="clear"
                    false-value="0" true-value="1" disable dense />
                </div>
                <div v-else-if="col.name==='is_verified'">
                  <q-toggle v-model="props.row.is_verified" checked-icon="check" color="green" unchecked-icon="clear"
                    false-value="0" true-value="1" disable dense />
                </div>
    ...
    <q-page-sticky expand position="top">
      <q-toolbar v-show="!stateform" class="main-toolbar">
        ...
        <div v-for="(btn,index) in btns" :key="index">
          <q-btn :dense="xs" flat no-caps :stack="xs" class="btn-toolbar q-mr-sm" :icon="btn.icon" :label="btn.caption"
