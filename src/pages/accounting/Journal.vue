  ...
  <q-page class="page-app">
    <div v-show="stateform">
      <q-card square class="icard">
        <q-bar dense class="entry-caption">
          ...
          <div class="row items-start q-col-gutter-xs q-mb-sm">
            <div class="col-xs-12 col-sm-6 col-md-6">
              ...
              <div class="row items-start q-col-gutter-xs q-mb-sm">
                <div class="col-xs-6 col-sm-6 col-md-5">
                  <q-input v-model="edit.reference1" dense outlined type="text" label="Referensi 1" square
                    stack-label />
                </div>
                <div class="col-xs-6 col-sm-6 col-md-5">
                  <q-input v-model="edit.reference2" dense outlined type="text" label="Referensi 2" square
                    stack-label />
                </div>
              </div>
              <div class="row items-start q-col-gutter-xs q-mb-sm">
            ...
            <div class="col-xs-12 offset-sm-1 col-sm-5 offset-md-2 col-md-4">
              <div class="row items-start q-col-gutter-xs q-mb-sm">
                <div class="col-6">
                  <q-field square outlined stack-label dense :model-value="edit.debit" label="Debit">
                    <template v-slot:control>
                      <vue-numeric v-model="edit.debit" class="q-field__input text-right" separator="."
                        :precision="2" />
                    </template>
                  </q-field>
                </div>
                <div class="col-6">
                  <q-field square outlined stack-label dense :model-value="edit.credit" label="Kredit">
                    <template v-slot:control>
                      <vue-numeric v-model="edit.credit" class="q-field__input text-right" separator="."
                        :precision="2" />
                    </template>
                  </q-field>
                </div>
              ...
              <div class="row items-start q-mb-sm q-col-gutter-xs q-mb-sm">
                <div class="col-6">
                  <q-input v-model="edit.fiscal_month" dense outlined readonly type="text" label="Bulan Fiscal"
                    square />
                </div>
                <div class="col-6">
        ...
        <q-table class="grid-tables" square :rows="detail" :columns="coldetail" dense no-data-label="data kosong"
          ...
          <template v-slot:top>
            <q-btn v-show="!posted && !deleted" label="Tambah" dense no-caps flat icon="add" class="text-primary"
              @click="addrow" />
            <img v-show="posted" src="statics/logo/Posted.png" spinner-color="white"
              style="height: 30px; max-width: 300px">
            <img v-show="deleted" src="statics/logo/Deleted.png" spinner-color="white"
              style="height: 30px; max-width: 300px">
          </template>
              ...
              <q-td v-for="col in props.cols" :key="col.name" :props="props">
                <div class="grid-data">
                  <div v-if="col.name === 'line_no'">
                    {{ col.value }}
                    <q-icon name="delete" color="red" size="xs" @click="removeRow(props.row.line_no)" />
                  </div>
                  <div v-else-if="col.name === 'no_account'">
                    <input v-model="props.row.no_account" type="text" style="width:80px" class="q-field__input input">
                    <q-icon name="search" color="blue" size="sm" @click="openAccount(props.row.line_no)" />
                  </div>
                  <div v-else-if="col.name === 'line_memo'">
                    <input v-model="props.row.line_memo" type="text" style="width:300px" class="q-field__input input">
                  </div>
                  <div v-else-if="col.name === 'reference1'">
                    <input v-model="props.row.reference1" type="text" style="width:150px" class="q-field__input input">
                  </div>
                  <div v-else-if="col.name === 'reference2'">
                    <input v-model="props.row.reference2" type="text" style="width:150px" class="q-field__input input">
                  </div>
                  <div v-else-if="col.name === 'debit'">
                    <vue-numeric v-model="props.row.debit" precision="2" thousand-separator="." style="width:80px"
                      class="q-field__input right-input" @input="calculate()" />
                  </div>
                  <div v-else-if="col.name === 'credit'">
                    <vue-numeric v-model="props.row.credit" precision="2" style="width:80px" thousand-separator="."
                      class="q-field__input right-input" @input="calculate()" />
                  </div>
                  <div v-else>
                    {{ col.value }}
    ...
    <q-card v-show="!stateform" square class="icard q-mb-sm">
      <q-bar class="entry-caption" style="font-size:10px">
        <span><strong>Filter</strong></span>
      </q-bar>
      <q-card-section class="q-pa-sm q-mb-xs">
        <div class="row items-center q-col-gutter-xs q-mb-xs">
          <div class="col-xs-6 col-sm-2 col-md-2">
            <q-input v-model="date1" outlined dense square type="date" stack-label />
          </div>
          <div class="col-xs-6 col-sm-2 col-md-2">
            <q-input v-model="date2" outlined dense square type="date" stack-label />
          </div>
          <div class="col-xs-3 col-sm-3 col-md-3">
            <q-select v-model="trans_code" :options="voucher_all" outlined dense options-dense label="Voucher"
              option-value="voucher_code" option-label="voucher_code" emit-value map-options fill-input square
              stack-label />
          </div>
          <div class="col-xs-8 col-sm-3 col-md-3">
            <q-select v-model="transtype" :options="jurnal_type" outlined dense options-dense label="Tipe Jurnal"
              option-value="jurnal_type" option-label="descriptions" emit-value map-options fill-input square />
          </div>
          <div class="col-xs-1 col-sm-1 col-md-2">
            <q-icon name="search" size="sm" color="primary" class="q-ml-sm" @click="loaddata()" />
          </div>
        </div>
      </q-card-section>
    </q-card>
    <q-card v-show="!stateform" square class="icard q-mb-sm">
      ...
      <q-table square class="grid-tables" :rows="data" :columns="columns" dense no-data-label="data kosong"
            ...
            <q-td v-for="col in props.cols" :key="col.name" :props="props">
              <div class="grid-data">
                ...
                <div v-else-if="col.name==='is_verified'">
                  <q-toggle v-model="props.row.is_verified" true-value="1" false-value="0" dense disable />
                </div>
                <div v-else>
                  {{ col.value }}
                </div>
    ...
    <q-page-sticky expand position="top">
      <q-toolbar v-show="!stateform" class="main-toolbar">
        <q-btn v-for="(btn, index) in btns" :key="index" dense no-caps flat v-show="btn.is_allowed" class="q-mr-xs"
          :icon="btn.icon" :label="btn.caption" @click="runMethod(btn.onclick)">
          <q-tooltip content-class="tooltips-app">
            {{ btn.tooltips }}
          </q-tooltip>
        </q-btn>
      ...
      <q-toolbar v-show="stateform" class="main-toolbar">
        <q-btn :label="lblSave" class="q-mr-xs" no-caps flat :disable="posted || deleted" icon="save" color="positive"
          @click="save_event()" />
        <q-btn label="Batal" icon="undo" no-caps color="negative" flat @click="stateform = false" />
      </q-toolbar>
    </q-page-sticky>
...
export default defineComponent({
  ...
  setup () {
    ...
    onMounted(async () => {
      ...
      voucher_all.value=[]
      voucher_all.value.push({voucher_code:'ALL',voucher_name:'SEMUA VOUCHER'})
