  ...
  <q-page class="page-app">
      ...
      <q-table square :rows="data" :columns="columns" dense no-data-label="data kosong"
        ...
        <template v-slot:body="props">
          <q-tr :props="props" @click="props.selected = !props.selected">
            ...
            :class="(props.row.is_group===false) ?'q-pa-xs text-black':'q-pa-xs text-green-10 text-bold'">
              <div class="grid-data">
                ...
                <div v-else-if="col.name === 'is_group'">
                  <q-toggle v-model="props.row.is_group" dense disable />
                </div>
    ...
    <q-page-sticky expand position="top">
      <q-toolbar class="main-toolbar">
        <q-btn v-for="(btn, index) in btns" :key="index" dense no-caps flat v-show="btn.is_allowed" class="btn-toolbar q-mr-xs"
      ...
      <q-card class="icard" style="width: 500px" square>
        ...
        <q-card-section class="q-pa-sm">
          <div class="row items-center q-col-gutter-sm">
            <div class="col-6">
              <q-input v-model="edit.user_name" dense outlined square type="text" label="User ID"
                stack-label />
            </div>
            <div class="col-6">
              <q-toggle v-model="edit.is_group" label="User group" color="blue" dense  stack-label />
            </div>
          </div>
          <div class="row items-center q-col-gutter-sm q-mt-xs">
            <div class="col-xs-12 col-sm-6">
              <q-input v-model="edit.full_name" outlined dense square label="Nama User" stack-label />
            </div>
            <div class="col-xs-12 col-sm-6">
              <q-select v-model="edit.user_level" :options="['ADMIN','SUPVR','USER']" options-dense outlined dense
                square label="Level User" stack-label />
            </div>
          </div>
          <div class="row items-start q-col-gutter-sm q-mt-xs">
            <div class="col-xs-12 col-sm-6 col-md-6">
              <q-input v-model="edit.email" outlined dense square label="email" stack-label />
            </div>
            <div class="col-xs-12 col-sm-6 col-md-6">
              <q-input v-model="edit.phone" outlined dense square type="text" label="Nomor Telepon" stack-label />
            </div>
          </div>
          <div class="row items-start q-col-gutter-sm q-mt-xs">
            <q-toggle v-model="edit.is_active" label="Aktif" color="blue" dense />
          </div>
        </q-card-section>
        <q-separator />
    ...
    <q-dialog v-model="pwdevent" persistent transition-show="flip-down" transition-hide="flip-up">
      <q-card class="icard" style="width: 500px" square>
        ...
        <q-card-section class="q-pa-sm q-gutter-xs">
          <div class="row items-center q-col-gutter-sm">
            <div class="col-xs-12 col-sm-6">
              <q-input v-model="edit.user_name" dense outlined readonly square type="text" label="User ID" />
            </div>
            <div class="col-xs-9 col-sm-6">
              <q-input v-model="edit.full_name" outlined dense readonly square type="text" label="Nama User" />
            </div>
          </div>
          <div class="row items-start q-col-gutter-sm q-mt-xs">
            <div class="col-12">
              <q-input v-model="pwd1" outlined dense square :type="isPwd ? 'password' : 'text'"
                hint="Password dengan toggle" label="Password">
                <template v-slot:append>
                  <q-icon :class="isPwd ? 'fas fa-eye-slash' : 'fas fa-eye'" @click="isPwd = !isPwd" size="xs"
                    color="green-10" />
                </template>
              </q-input>
            </div>
          ...
          <div class="row items-start q-col-gutter-sm q-mt-xs">
            <div class="col-12">
              <q-input v-model="pwd2" outlined dense square :type="isPwd ? 'password' : 'text'"
                hint="Password dengan toggle" label="Konfirmasi Password" />
            </div>
          </div>
        </q-card-section>
        <q-separator />
