  ...
  <q-page class="page-app">
    ...
    <q-card v-show="!stateform" square class="icard">
      ...
      <q-table square :rows="data" :columns="columns" no-data-label="data kosong" :dense="$q.screen.md"
        no-results-label="data yang kamu cari ttransidak ditemukan" row-key="transid" :filter="filter" separator="cell"
            ...
            <q-td v-for="col in props.cols" :key="col.name" :props="props">
              <div class="grid-data">
                ...
                <div v-else-if="col.name === 'order_no'">
                  <span
                    :class="(props.row.order_status==='Close')? 'q-pa-xs bg-green text-white' : (props.row.order_status==='Cancel') ? 'q-pa-xs bg-red-10 text white':'bg-white text-black'">{{props.row.order_no}}</span>
                </div>
                <div v-else-if="col.name === 'work_order_no'">
                  <span
                    :class="(props.row.is_closed_expense==='1')? 'q-pa-xs bg-green text-white' : (props.row.order_status==='Cancel') ? 'q-pa-xs bg-red-10 text white':'bg-white text-black'">{{
                    props.row.work_order_no }}</span>
                </div>
                <div v-else-if="col.name === 'is_closed_order'">
                  <q-checkbox v-model="props.row.is_closed_order" true-value="1" false-value="0" dense disable />
                </div>
                <div v-else-if="col.name === 'is_closed_expense'">
                  <q-checkbox v-model="props.row.is_closed_expense" true-value="1" false-value="0" dense disable />
                </div>
    ...
    <q-card v-show="stateform" class="icard" square>
      ...
      <q-card-section class="q-gutter-sm">
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          ...
          <div class="col-xs-6 col-sm-4 col-md-2">
            <q-select v-model="edit.order_type" dense outlined square label="Tipe Order" stack-label
              :options="['RITASE','TONASE']" options-dense />
          </div>
        </div>
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          <div class="col-xs-6 col-sm-4 col-md-2">
            <q-input v-model="edit.order_date" dense outlined square label="Tgl.Order" stack-label type="date" />
          </div>
          <div class="col-xs-6 col-sm-4 col-md-2">
            <q-input v-model="edit.order_time" dense outlined square label="Jam.Order" stack-label type="time" />
          </div>
          <div class="col-xs-12 col-sm-4 col-md-2">
            <q-select v-model="edit.work_type" dense outlined square label="Tipe Pekerjaan" stack-label
              :options="['EXPORT','IMPORT','DOMESTIC','CARGO','POK']" options-dense />
          </div>
        </div>
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          <div class="col-xs-12 col-sm-8 col-md-3">
            <q-input v-model="edit.partner_name" dense outlined square label="Nama Konsumen" stack-label
              placeholder="Klik tombol untuk mencari nama konsumen" readonly>
              <template v-slot:append>
        ...
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          <div class="col-xs-12 col-sm-6 col-md-3">
            <q-input v-model="edit.route_name" dense outlined square label="Rute operasional" stack-label
              placeholder="Klik tombol untuk mencari rute operasional" readonly>
              <template v-slot:append>
                <q-icon name="search" color="blue-grey-10" size="xs" @click="Open_routes()" />
              </template>
            </q-input>
          </div>
          <div class="col-xs-12 col-sm-6 col-md-3">
        ...
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          ...
          <div class=" col-xs-4 col-sm-4 col-md-2">
            <q-field square outlined stack-label dense :model-value="edit.qty" label="Jumlah">
              <template v-slot:control>
                <vue-numeric v-model="edit.qty" class="q-field__input text-right" separator="." :precision="0"
                  @input="edit.standart_price = edit.price * edit.qty" />
              </template>
            </q-field>
          </div>
          <div class=" col-xs-4 col-sm-4 col-md-2">
            <q-field square outlined stack-label dense :model-value="edit.standart_price" label="Net Tarif">
              <template v-slot:control>
        ...
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          <div class="col-xs-4 col-sm-4 col-md-2">
            <q-input v-model="edit.planning_date" dense outlined square label="Tgl.Kegiatan" stack-label type="date" />
          </div>
          <div class="col-xs-4 col-sm-4 col-md-2">
            <q-input v-model="edit.planning_time" dense outlined square label="Jam Kegiatan" stack-label type="time" />
          </div>
          <div class=" col-xs-4 col-sm-4 col-md-2">
            <q-field square outlined stack-label dense :model-value="edit.fleet_cost" label="Biaya Operasional">
              <template v-slot:control>
                <vue-numeric v-model="edit.fleet_cost" class="q-field__input text-right" separator="." :precision="0"
                  disable />
              </template>
            </q-field>
          </div>
        </div>
        <div class="row items-start q-col-gutter-sm q-mb-sm" v-show="edit.order_no==='(NEW)'">
          <div class=" col-xs-4 col-sm-4 col-md-2">
            <q-field square outlined stack-label dense :model-value="edit.order_qty" label="Jml. Order">
              <template v-slot:control>
                <vue-numeric v-model="edit.order_qty" class="q-field__input text-right" separator="." :precision="0"
                  disable />
              </template>
            </q-field>
          </div>
        ...
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          <div class="col-xs-12 col-sm-12 col-md-6">
            <q-input v-model="edit.descriptions" dense outlined square label="Keterangan" stack-label type="textarea" />
          </div>
        </div>
      </q-card-section>
    </q-card>
    ...
    <q-dialog v-model="dlgCancel" persistent>
      <q-card class="icard" style="min-width:600px">
        <q-bar class="entry-caption">Pembatalan reservasi
          <q-space />
        ...
        <q-card-section class="q-gutter-sm">
          ...
          <div class="row items-start q-col-gutter-sm q-mb-sm">
            <div class="col-4">
              <q-input v-model="edit.order_date" dense outlined square label="Jam Order" stack-label type="date"
                disable />
            </div>
            <div class="col-4">
              <q-input v-model="edit.order_time" dense outlined square label="Tgl.Order" stack-label type="time"
                disable />
            </div>
            <div class="col-4">
              <q-select v-model="edit.work_type" dense outlined square label="Tipe Pekerjaan" stack-label
        ...
        <q-card-section class="q-pa-sm entry-caption" align="right">
          <q-btn label="Pembatalan reservasi" no-caps dense glossy icon="cancel" @click="cancel_order()" />
        </q-card-section>
      </q-card>
    </q-dialog>

    <q-dialog v-model="dlgRoute" persistent>
      <q-card class="icard" style="min-width:600px">
        <q-bar class="entry-caption">Route Operasional - {{edit.partner_name}}
          <q-space />
          <q-input v-model="filter_routes" dense outline debounce="300" label-color="white" borderless
        ...
        <q-table square :rows="routes" :columns="col_routes" no-data-label="data kosong"
          no-results-label="data yang kamu cari tidak ditemukan" row-key="id" separator="cell" selection="single"
          virtual-scroll dense class="fit" table-style="min-height:302px; max-height: 350px"
          v-model:selected="sel_routes" v-model:pagination="pg_routes" binary-state-sort @request="onRequestRoutes"
          :loading="loading_routes" :filter="filter_routes">
          <template v-slot:no-data="{ icon, message, filter }">
          ...
          <template v-slot:body="props">
            <q-tr :props="props" @click="props.selected = !props.selected">
              <q-td v-for="col in props.cols" :key="col.name" :props="props">
                <div class="grid-data">
                  <div v-if="((col.name === 'origins') || (col.name==='destinations'))">
                    <q-btn :label=" col.value" no-caps dense flat @click="change_routes(props.row.id)" />
                  </div>
        ...
        <q-card-section class="entry-caption q-pa-sm" align="right">
          <q-btn no-caps label="Pilih" color="positive" icon="check" flat class="q-mr-sm" @click="change_routes(-1)" />
          <q-btn no-caps color="negative" label="Tutup" icon="close" flat v-close-popup />
        </q-card-section>
      </q-card>
    </q-dialog>
...
export default defineComponent({
  ...
  setup() {
    ...
    const col_routes=ref([
      { name:'vehicle_type',align:'left',label:'Type Unit',field:'vehicle_type',sortable:true},
      { name: 'origins', align: 'left', label: 'Asal', field: 'origins', sortable: true },
      { name: 'destinations', align: 'left', label: 'Tujuan', field:'destinations',sortable:true},
      { name: 'fleet_price', align: 'right', label: 'Tarif', field:'fleet_price'},
      { name:'eta',align:'right',label:'ETA',field:'eta'},
      { name: 'standart_cost', align: 'right', label: 'Biaya Ops', field: 'standart_cost' }
    ])
    ...
    async function add_event() {
        ...
        edit.value = {
          ...
          order_time: skrng.toLocaleTimeString('en-CA', { hour12: false, timeStyle: 'short' }),
          planning_date: skrng.toLocaleDateString("en-CA"),
          planning_time: skrng.toLocaleTimeString('en-CA', { hour12: false, timeStyle: 'short' }),
          warehouse:'',
    ...
    function change_routes (id=-1) {
      ...
      routes.value.forEach(el=>{
        if (el.id === id_route){
          ...
          edit.value.route_name = el.vehicle_type + '(' + el.origins+' - '+el.destinations+' )'
        }
      })
