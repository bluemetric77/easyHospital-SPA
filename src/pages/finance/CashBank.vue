            ...
            >
              <div class="grid-data">
                ...
                <div v-else-if="col.name === 'descriptions'">
                  <q-input
                    v-model="props.row.descriptions"
                    type="textarea"
                    autogrow
...
export default defineComponent({
  ...
  setup() {
    ...
    function ujo_event(sysid = -1) {
      $router.push({
        path: '/finance/cash-bank/ujo',
        query: { sysid: -1, state: 'new' }
      })
    }
    function droping_event(sysid = -1) {
      $router.push({
        path: '/finance/cash-bank/droping',
        query: { sysid: -1, state: 'new' }
      })
    }
    function receiving_event(sysid = -1) {
      $router.push({
        path: '/finance/cash-bank/receive-droping',
        query: { sysid: -1, state: 'new' }
      })
    }
    function out_event(sysid = -1) {
      $router.push({
        path: '/finance/cash-bank/transaction-out',
        query: { sysid: -1, state: 'new' }
      })
    }
    function in_event(sysid = -1) {
      $router.push({
        path: '/finance/cash-bank/transaction-in',
        query: { sysid: -1, state: 'new' }
      })
    }
    ...
    function edit_cashbank(transid = -1, document_type = '', sysid = -1) {
      if (document_type === 'UJO') {
        $router.push({
          path: '/finance/cash-bank/ujo',
          query: { sysid: transid, state: 'update' }
        })
      } else if (document_type === 'DRO') {
        $router.push({
          path: '/finance/cash-bank/droping',
          query: { sysid: sysid, state: 'update' }
        })
    ...
    async function print_event(primary = -1) {
      if (selected.value.length > 0 || !(primary === -1)) {
        ...
        if (row.document_type === 'DRO' || row.document_type === 'RDO') {
          let props = {}
