            ...
            <q-td v-for="col in props.cols" :key="col.name" :props="props">
              <div class="grid-data">
                <div v-if="col.name === 'action'">
                  <q-icon class="q-mr-sm" name="edit" size="xs" color="green-10"
                    @click="edit_cashbank(props.row.linkdocument_id,props.row.document_type,props.row.transid)">
                ...
                <div v-else-if="col.name === 'linkdoc_number'">
                  <div v-if="(props.row.no_jurnal===-1)">
                    <span>BELUM DIJURNAL</span>
                  </div>
                  <div v-else>
                    <q-btn :label="props.row.voucher" no-caps flat rounded class="btn-operation"
                    @click="show_jurnal(props.row.no_jurnal)" />
                    <q-btn icon="print" no-caps flat rounded class="btn-operation"  @click="print_gl(props.row.no_jurnal)" />
                  </div>
                </div>
                <div v-else-if="col.name === 'descriptions'">
                  <q-input v-model="props.row.descriptions" type="textarea" autogrow input-style="max-width:350px"
                    borderless readonly />
                </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    async function loaddata() {
        ...
        $q.sessionStorage.set("cashbank_id", cash_id.value);
        $q.sessionStorage.set("date1", date1.value);
        $q.sessionStorage.set("date2", date2.value);
    }
    ...
    function droping_event(sysid=-1) {
      $router.push({ path: '/finance/cash-bank/droping', query: { sysid: -1,state:'new' } });
    }
    function receiving_event(sysid=-1) {
      $router.push({ path: '/finance/cash-bank/receive-droping', query: { sysid: -1,state:'new' } });
    }
    function out_event(sysid=-1) {
      $router.push({ path: '/finance/cash-bank/transaction-out', query: { sysid: -1,state:'new' } });
    }
    function in_event(sysid=-1) {
      $router.push({ path: '/finance/cash-bank/transaction-in', query: { sysid: -1,state:'new' } });
    }

    function edit_cashbank(transid=-1,document_type='',sysid=-1){
      if (document_type==='UJO'){
        $router.push({ path: '/finance/cash-bank/ujo', query: { sysid: transid, state: 'update' } });
      } else if (document_type==='DRO'){
        $router.push({ path: '/finance/cash-bank/droping', query: { sysid: sysid, state: 'update' } });
      } else if (document_type==='RDO'){
        $router.push({ path: '/finance/cash-bank/receive-droping', query: { sysid: sysid, state: 'update' } });
      } else if (document_type==='BOU'){
        $router.push({ path: '/finance/cash-bank/transaction-out', query: { sysid: transid, state: 'update' } });
      } else if (document_type==='BIN'){
        $router.push({ path: '/finance/cash-bank/transaction-in', query: { sysid: transid, state: 'update' } });
      }
    }
    ...
    async function print_event (primary = -1) {
      if (selected.value.length > 0 || !(primary === -1)) {
        ...
        if ((row.document_type==='DRO')  || 
            (row.document_type==='RDO')){
          let props = {};
          props.sysid = row.transid;
          ...
          props.document_type=row.document_type
    ...
    onMounted(async () => {
      ...
      date2.value= skrng.toLocaleDateString("en-CA");
        let icash_id= $q.sessionStorage.getItem("cashbank_id");
        let idate1 = $q.sessionStorage.getItem("date1", date1.value);
        let idate2 =$q.sessionStorage.getItem("date2", date2.value);
        cash_id.value = icash_id
        if (!(idate1===null)) {
          date1.value=idate1
        }       
        if (!(idate2===null)) {
