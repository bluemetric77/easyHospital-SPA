  ...
  <q-page class="page-app">
    ...
    <q-card class="icard" square>
      <q-bar class="entry-caption">
        Pengeluaran Kas/Bank - {{(stateform==='new') ?' Dokumen Baru' : edit.trans_code+'-'+edit.trans_series}}
      </q-bar>
          ...
          <q-tr :props="props" @click="props.selected = !props.selected">
            <q-td v-for="col in props.cols" :key="col.name" :props="props">
              <div class="grid-data">
                ...
                <div v-else-if="col.name === 'enum_type'" style="width=100%">
                  <select v-model="props.row.eum_type" class="input">
                    <option>-</option>
                    <option>KASBON BARANG</option>
                  </select>
                </div>
                <div v-else>
                  {{ col.value }}
...
export default defineComponent({
  ...
  setup () {
    ...
    const columns = ref([
      ...
      { name: 'enum_type', align: 'left', label: 'Tipe', field: 'enum_type' },
      { name: 'no_account', align: 'left', label: 'Akun', field: 'no_account',style:"min-width:100px"},
      { name: 'partner_id', align: 'left', label: 'BPartner', field: 'partner_id', style: "min-width:150px" }
    ]);
    ...
    async function save_data () {
      try {
        ...
        app.url = 'finance/cash_bank/transaction-out';
    ...
    function removeRow (lineno) {
      $q.dialog({
          dark: false,
          title: 'Konfirmasi',
          message: 'Apakah line penerimaan ini akan dihapus ?',
          cancel: true,
          persistent: true
        })
        .onOk(() => {
