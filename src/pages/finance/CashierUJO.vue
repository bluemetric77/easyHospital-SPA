  ...
  <q-page class="page-app">
    ...
    <q-card class="icard" square>
      <q-bar class="entry-caption">
        Pengeluran UJO - {{(stateform==='new') ?' Dokumen Baru' : edit.doc_number}}
      </q-bar>
      <q-card-section class="q-gutter-sm">
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          <div class="col-xs-6 col-sm-6 col-md-2">
            <q-select v-model="edit.pool_code" :options="pools" outlined dense options-dense label="Pool"
          ...
          <div class="col-xs-6 col-sm-6 col-md-2">
            <q-input v-model="edit.expense_no" dense outlined square label="LBO" stack-label readonly>
              <template v-slot:append>
                <q-icon name="search" size="xs" color="green-10" @click="OpenLBO()" v-show="edit.no_jurnal===-1" />
              </template>
            </q-input>
          </div>
        ...
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          <div class="col-xs-12 col-sm-6 col-md-4">
            <q-select v-model="edit.cash_id" :options="cashbanks" outlined dense options-dense label="Kas/Bank"
        ...
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          <div class="col-xs-6 col-sm-3  col-md-2">
            <q-field square outlined stack-label dense :model-value="edit.standart" label="LBO Standar">
              <template v-slot:control>
                <vue-numeric v-model="edit.standart" class="q-field__input text-right" separator="." />
              </template>
            </q-field>
          </div>
          <div class="col-xs-6 col-sm-3 col-md-2">
            <q-field square outlined stack-label dense :model-value="edit.dp_customer" label="DP Konsumen">
              <template v-slot:control>
                <vue-numeric v-model="edit.dp_customer" class="q-field__input text-right" separator="." />
              </template>
            </q-field>
          </div>
          <div class="col-xs-6 col-sm-3 offset-md-4 col-md-2">
            <q-field square outlined stack-label dense :model-value="edit.cashier" label="Total LBO">
              <template v-slot:control>
                <vue-numeric v-model="edit.cashier" class="q-field__input text-right" separator="."
                  @input="edit.total=edit.expense+edit.adm_fee" />
              </template>
            </q-field>
          </div>
          <div class="col-xs-6 col-sm-3 col-md-2">
            <q-input v-model="edit.dp_method" dense outlined square label="Cara DP" stack-label readonly />
          </div>
        </div>
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          <div class="col-xs-6 col-sm-3  col-md-2">
            <q-input v-model="edit.project_id" dense outlined square label="Proyek" stack-label readonly />
          </div>
          <div class="col-xs-6 col-sm-3 col-md-2">
            <q-field square outlined stack-label dense :model-value="edit.expense" label="Biaya">
              <template v-slot:control>
                <vue-numeric v-model="edit.expense" class="q-field__input text-right" separator="." />
              </template>
            </q-field>
          </div>
          <div class="col-xs-6 col-sm-3 offset-md-4 col-md-2">
            <q-field square outlined stack-label dense :model-value="edit.adm_fee" label="By.Admin">
              <template v-slot:control>
                <vue-numeric v-model="edit.adm_fee" class="q-field__input text-right" separator="."
                  @input="edit.total=edit.expense+edit.adm_fee" />
              </template>
            </q-field>
          </div>
          <div class="col-xs-6 col-sm-3 col-md-2">
            <q-field square outlined stack-label dense :model-value="edit.total" label="Total">
              <template v-slot:control>
                <vue-numeric v-model="edit.total" class="q-field__input text-right" separator="." readonly />
              </template>
            </q-field>
          </div>
        </div>
      </q-card-section>
      <q-table square :rows="costs" :columns="columns" no-data-label="data kosong" dense
        no-results-label="data yang kamu cari tidak ditemukan" row-key="line_no" separator="cell" selection="single"
        v-model:selected="selected_cost" v-model:pagination="pagination" binary-state-sort virtual-scroll
        hide-pagination>
          ...
          <q-tr :props="props">
            <q-th v-for="col in props.cols" :key="col.name" :props="props" class="bg-teal-10 text-white">
              <div v-if="col.name==='budget'">
                <q-btn label="Semua" glossy class="bg-red-14 text-white q-mr-sm" rounded dense no-caps
                  @click="expense(-1,1)" />{{col.label}}
              </div>
              <div v-else>
                {{ col.label }}
              </div>
            ...
            <q-td v-for="col in props.cols" :key="col.name" :props="props">
              <div class="grid-data">
                <div v-if="col.name === 'budget'">
                  ...
                  <div v-else>
                    <q-btn :label="$formatnumber(props.row.budget)" rounded glossy class="bg-red-14 text-white"
                ...
                <div v-else-if="col.name === 'expense'">
                  <q-field square outlined stack-label dense :model-value="props.row.expense">
                    <template v-slot:control>
                      <vue-numeric v-model="props.row.expense" class="q-field__input text-right" separator="."
    ...
    <q-dialog v-model="dlglbo" persistent full-width>
      <q-card class="icard" square>
        <q-bar class="entry-caption">
          Pengajuan Biaya Operasional - {{edit.pool_code}}
          <q-space />
          <q-input v-model="filter_lbo" dense outline debounce="300" label-color="white" borderless
            placeholder="Pencarian" input-class="text-white" />
          <q-btn icon="close" color="red-10" size="xs" dense glossy rounded v-close-popup />
        </q-bar>
        <q-table square :rows="lbos" :columns="columns_lbo" no-data-label="data kosong" dense
          no-results-label="data yang kamu cari tidak ditemukan" row-key="transid" :filter="filter_lbo" separator="cell"
          selection="single" v-model:selected="selected_lbo" v-model:pagination="pagination_lbo"
          class="fit my-sticky-dynamic" table-style="min-height:302px; max-height: 370px" :virtual-scroll-item-size="48"
          :virtual-scroll-sticky-size-start="48" binary-state-sort virtual-scroll :loading="loading"
              ...
              <q-td v-for="col in props.cols" :key="col.name" :props="props">
                <div class="grid-data">
                  <div v-if="col.name==='doc_number'">
                    <q-btn :label="props.row.doc_number" glossy dense no-caps rounded class="bg-green-10 text-white"
...
export default defineComponent({
  ...
  setup () {
    ...
    const columns = ref([
      ...
      { name: 'expense_out', align: 'right', label: 'Realisasi', field: 'expense_out' },
      { name: 'expense', align: 'right', label: 'Biaya', field: 'expense',style:"min-width:100px"},
      { name: 'line_note', align: 'left', label: 'Catatan', field: 'line_note', style: "min-width:150px" },
    ...
    const columns_lbo = ref([
      { name: 'doc_number', align: 'center', label: 'No.LBO', field: 'doc_number',sortable:true },
      ...
      { name: 'driver_name', align: 'left', label: 'Pengemudi', field: 'driver_name', sortable: true },
      { name: 'vehicle_no', align: 'left', label: 'No. Unit', field: 'vehicle_no', sortable: true },
    ...
    function expense(line=-1,all=0) {
      ...
      costs.value.forEach(el=>{
        ...
        if ((el.line_no===line) || (all===1)){
          costs.value[i].expense = costs.value[i].budget - costs.value[i].expense_out
        }
      })
      calculate_cost()
    }
