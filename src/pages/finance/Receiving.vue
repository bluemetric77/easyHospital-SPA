  ...
  <q-page class="page-app">
    ...
    <q-card class="icard" square>
      <q-bar class="entry-caption">
        Penerimaan droping UJO/Bank - {{(stateform==='new') ?' Dokumen Baru' : edit.doc_number}}
      </q-bar>
      <q-card-section class="q-gutter-sm">
        ...
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          <div class="col-xs-12 col-sm-6 col-md-4">
            <q-select v-model="edit.cashbank_source" :options="cashbanks_source" outlined dense options-dense label="Diterima oleh"
              option-value="cash_id" option-label="descriptions" emit-value map-options fill-input stack-label square/>
          </div>
        </div>
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          <div class="col-xs-12 col-sm-6 col-md-4">
            <q-select v-model="edit.cashbank_destination" :options="cashbanks_source" outlined dense options-dense label="Dikirim oleh"
              option-value="cash_id" option-label="descriptions" emit-value map-options fill-input stack-label square/>
          </div>
        </div>
        <div class="row items-start q-col-gutter-sm q-mb-sm">
    ...
    <q-page-sticky expand position="top">
      <q-toolbar class="main-toolbar">
        <q-btn dense no-caps flat class="btn-toolbar q-mr-xs" icon="fas fa-coins" label="Droping UJO" @click="show_droping()">
    ...
    <q-dialog v-model="dlgDroping" persistent>
      <q-card square class="icard">
        <q-bar class="entry-caption">List Droping UJO
          <q-space/>
        ...
        <q-table square :rows="droping" :columns="columns" no-data-label="data kosong" class="my-sticky-table" dense
          no-results-label="data yang kamu cari tidak ditemukan" row-key="transid" separator="cell" selection="single"
          v-model:selected="selected" v-model:pagination="pagination" binary-state-sort virtual-scroll :filter="filter"
        ...
        <q-card-section class="entry-caption q-pa-sm" align="right">
          <q-btn no-caps label="Pilih" icon="check" flat class="q-mr-sm" @click="selectdata()" />
          <q-btn no-caps label="Tutup" icon="close" flat v-close-popup />
        </q-card-section>
      </q-card>
    </q-dialog>
  </q-page>
...
export default defineComponent({
  ...
  setup () {
    ...
    const selected = ref([]);
    const columns = ref([
      { name: 'doc_number', align: 'center', label: 'Dokumen Droping', field: 'doc_number' },
      ...
      { name: 'cashbank_sname', align: 'right', label: 'Kas/Bank Pengirim', field: 'cashbank_sname'}
    ]);
    ...
    function selectdata(){
      if (selected.value.length>0) {
        ...
        edit.value.descriptions='PENERIMAAN DROPING '+data.ref_date+' NO BUKTI '+data.doc_number
      }
      change_payment()
