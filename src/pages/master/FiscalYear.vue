  ...
  <q-page class="page-app">
    <q-card square class="icard q-mb-sm">
      ...
      <q-card-section class="q-pa-sm">
        <div class="row">
          <q-input v-model="year_period" label="Periode Akunting" dense outlined stack-label square style="width:300px">
            <template v-slot:append>
              <q-btn label="Cek data" glossy rounded dense no-caps color="blue-grey-10" @click="loaddata()" />
            </template>
          </q-input>
          <q-space />
          <q-btn label="Simpan data" glossy rounded dense no-caps color="blue-grey-10" @click="save_data()" />
        </div>
      </q-card-section>
    </q-card>
    <q-card square class="icard">
      ...
      <q-table square :rows="data" :columns="columns" no-data-label="data kosong" :dense="$q.screen.md"
        no-results-label="data yang kamu cari tidak ditemukan" row-key="fiscal_id" :filter="filter" separator="cell"
            ...
            <q-td v-for="col in props.cols" :key="col.name" :props="props">
              <div class="grid-data">
                ...
                <div v-else-if="col.name === 'is_closed'">
                  <q-checkbox v-model="props.row.is_closed" dense true-value="1" false-value="0" />
                </div>
                <div v-else-if="col.name === 'start_date'">
                  <q-input v-model="props.row.start_date" dense outlined square type="date" />
                </div>
                <div v-else-if="col.name === 'end_date'">
                  <q-input v-model="props.row.end_date" dense outlined square type="date"/>
                </div>
