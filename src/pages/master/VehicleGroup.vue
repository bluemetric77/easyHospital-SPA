  ...
  <q-page class="page-app">
    <q-card square class="icard">
      ...
      <q-table square :rows="data" :columns="columns" no-data-label="data kosong" :dense="$q.screen.md"
        no-results-label="data yang kamu cari tidak ditemukan" row-key="vehicle_type" :filter="filter" separator="cell"
              ...
              <div class="grid-data">
                <div v-if="col.name === 'action'">
                  <q-icon v-for="(btn, index) in btns" v-show="btn.is_allowed && btn.is_show" :key="index" no-caps flat
                    ...
                    @click="runMethod(btn.onclick, props.row.vehicle_type)">
      ...
      <q-card class="icard" style="width: 700px" square>
        ...
        <q-card-section class="q-gutter-sm">
          <div class="row items-start q-col-gutter-sm q-mb-sm">
            <div class="col-4">
              <q-input v-model="edit.vehicle_type" dense outlined square label="Type" stack-label />
            </div>
            <div class="col-8">
              <q-input v-model="edit.descriptions" dense outlined square label="Type kendaraam" stack-label />
            </div>
          </div>
          <div class="row items-start q-col-gutter-sm q-mt-xs">
            <div class="col-12">
              <q-select v-model="edit.main_group" 
              :options="['BAK','BOX','MINIBUS','TRAILER']" outlined dense options-dense label="Grup Utama"
    ...
    async function edit_event(primary ='') {
      if (selected.value.length > 0 || !(primary === '')) {
        ...
        try {
          ...
          props.vehicle_type = primary;
          let respon = await $store.dispatch("master/GET_DATA", props);
          if (!(typeof respon === "undefined")) {
            dataevent.value = true;
            edit.value = respon;
            edit.value.old_vehicle_type=respon.vehicle_type
          }
        } finally {
          $q.loading.hide();
        }
