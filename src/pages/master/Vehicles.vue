  ...
  <q-page class="page-app">
    <q-card square class="icard">
      ...
      <q-table square :rows="data" :columns="columns" no-data-label="data kosong" :dense="$q.screen.md"
        no-results-label="data yang kamu cari tidak ditemukan" row-key="vehicle_no" :filter="filter" separator="cell"
            ...
            <q-td v-for="col in props.cols" :key="col.name" :props="props">
              <div class="grid-data">
                ...
                <div v-else-if="col.name === 'vehicle_no'">
                  <span v-if="props.row.is_active==='1'">{{ props.row.vehicle_no }}</span>
                  <span class="text-red-10" v-else>{{ props.row.vehicle_no }}</span>
                </div>
                <div v-else-if="col.name === 'descriptions'">
                  <span v-if="props.row.is_active==='1'">{{ props.row.descriptions }}</span>
                  <span class="text-red-10" v-else>{{ props.row.descriptions }}</span>
                </div>
      ...
      <q-card class="icard" style="width: 700px" square>
        ...
        <q-card-section class="q-pa-xs">
          <div class="row items-center q-col-gutter-sm q-mb-xs">
            <div class="col-3">
              <q-input v-model="edit.vehicle_no" dense outlined square label="No.Unit" stack-label />
            </div>
            <div class="col-7">
              <q-input v-model="edit.descriptions" dense outlined square label="Nama unit/kendaraan" stack-label />
            </div>
            <div class="col-2">
          ...
          <div class="row items-start q-col-gutter-sm">
            <div class="col-3">
              <q-select v-model="edit.gps_vendor" :options="['N/A','TOTAL KILAT','EASY GO']" outlined dense
                options-dense label="Vendor GPS" fill-input stack-label square />
            </div>
            <div class="col-9">
              <q-select v-model="edit.gps_id" :options="gps_devices" option-value="device_id" option-label="list"
                map-options emit-value outlined dense options-dense label="ID GPS" fill-input stack-label square />
            </div>
          </div>
        </q-card-section>
        <q-tabs v-model="tab" class="text-teal" dense inline-label no-caps align="justify">
          <q-tab name="general" icon="fas fa-address-card" label="Umum" />
          <q-tab name="legal" icon="fas fa-file-contract" label="Perizinan/Dokumen" />
        </q-tabs>
        <q-tab-panels v-model="tab" animated swipeable vertical transition-prev="jump-up" transition-next="jump-up">
          <q-tab-panel name="general">
            <div class="row items-start q-col-gutter-sm q-mb-xs">
              <div class="col-4">
                <q-input v-model="edit.police_no" dense outlined square label="No. Polisi" stack-label />
              </div>
              <div class="col-4">
                <q-input v-model="edit.model" dense outlined square label="Model" stack-label />
              </div>
              <div class="col-4">
                <q-input v-model="edit.manufactur" dense outlined square label="ATPM" stack-label />
              </div>
            </div>
            <div class="row items-start q-col-gutter-sm q-mb-xs">
              <div class="col-4">
                <q-input v-model="edit.vin" dense outlined square label="Nomor Mesin" stack-label />
              </div>
              <div class="col-4">
                <q-input v-model="edit.chasis_no" dense outlined square label="Nomor Rangka" stack-label />
              </div>
              <div class="col-4">
                <q-input v-model="edit.year_production" dense outlined square label="Tahun Perakitan" stack-label />
              </div>
            </div>
            <div class="row items-start q-col-gutter-sm q-mb-xs">
              <div class="col-6">
                <q-select v-model="edit.unit_type" :options="['FULL UNIT','HEAD UNIT','CHASSIS']" outlined dense
                  options-dense label="Kategori kendaraan/unit" fill-input stack-label square />
              </div>
              <div class="col-6">
            ...
            <div class="row items-start q-col-gutter-sm q-mb-xs">
              <div class="col-6">
                <q-input v-model="edit.tire_type" dense outlined square label="Jenis Ban" stack-label />
              </div>
              <div class="col-6">
            ...
            <div class="row items-start q-col-gutter-sm q-mb-xs">
              <div class="col-12">
                <q-input v-model="edit.personal_name" dense outlined square label="Pengemudi" stack-label readonly>
                  <template v-slot:prepend>
                    <q-icon name="clear" color="red-10" size="xs" @click="edit.personal_name=''" />
                  </template>
                  <template v-slot:append>
                    <q-icon name="search" color="teal-10" size="xs" @click="open_driver()" />
                  </template>
                </q-input>
              </div>
            </div>
          ...
          <q-tab-panel name="legal">
            <div class="row items-start q-col-gutter-sm q-mb-xs">
              <div class="col-4">
                <q-input v-model="edit.stnk_validate" dense outlined square label="Masa berlaku STNK" type="date"
                  stack-label />
              </div>
              <div class="col-4">
                <q-input v-model="edit.vehicle_tax_validate" dense outlined square label="Masa berlaku pajak"
                  type="date" stack-label />
              </div>
              <div class="col-4">
                <q-input v-model="edit.kir_validate" dense outlined square label="Masa berlaku KIR" type="date"
                  stack-label />
              </div>
            </div>
            <div class="row items-start q-col-gutter-sm q-mb-xs">
              <div class="col-4">
                <q-input v-model="edit.purchase_date" dense outlined square label="Tgl. Pembelian" type="date"
                  stack-label />
              </div>
              <div class="col-4">
                <q-field square outlined stack-label dense :model-value="edit.purchase_amount" label="Nilai Pembelian">
                  <template v-slot:control>
                    <vue-numeric v-model="edit.purchase_amount" class="q-field__input text-right" separator="."
                      :precision="0" />
                  </template>
                </q-field>
              </div>
            ...
            <div class="row items-start q-col-gutter-sm q-mb-xs">
              <div class="col-4">
                <q-input v-model="edit.asset_number" dense outlined square label="Nomor Asset" stack-label />
              </div>
              <div class="col-4">
                <q-field square outlined stack-label dense :model-value="edit.asset_value" label="Nilai Asset">
                  <template v-slot:control>
                    <vue-numeric v-model="edit.asset_value" class="q-field__input text-right" separator="."
                      :precision="0" readonly />
                  </template>
                </q-field>
              </div>
            </div>
    ...
    <q-dialog v-model="documentevent" persistent>
      <q-card class="icard" style="width: 800px" square>
        <q-bar class="entry-caption">Dokumen
          <q-space />
          <q-btn v-close-popup dense glossy rounded icon="close" color="red-5" size="xs">
            <q-tooltip>Close</q-tooltip>
          </q-btn>
        ...
        <q-card-section class="q-pa-xs">
          ...
          <div class="row items-start q-col-gutter-sm">
            ...
            <div class="col-4">
              <q-input v-model="edit.chasis_no" dense outlined square label="Nomor rangka kendaraan" stack-label />
            </div>
          </div>
        </q-card-section>
        <q-toolbar class="main-toolbar">
          <q-btn label="Upload dokumen" icon="fas fa-file-upload" dense flat no-caps @click="upload_document()" />
        </q-toolbar>
        <q-card-section class="q-pa-xs">
          <q-scroll-area style="height: 350px">
            <div class="row items-start q-col-gutter-xs q-mb-xs">
              <div class="col-xs-4" v-for="(item,index) in documents" :key="index">
                <q-card class="shadow-5 overflow-hidden" style="border-radius: 15px">
                  <q-img :src="item.url_link" :ratio="4/3">
                    <q-icon class="absolute all-pointer-events" size="xs" name="fas fa-search-plus" color="green-10"
                      style="top: 8px; left: 8px" @click="preview_image(item.sysid)">
                      <q-tooltip>
                        Preview
                      </q-tooltip>
                    </q-icon>
                    <q-icon class=" absolute all-pointer-events" size="xs" name="fas fa-trash" color="red-10"
                      style="top: 8px; left: 40px" @click="remove_image(item.sysid)">
                      <q-tooltip>
                        Hapus
                      </q-tooltip>
                    </q-icon>
    ...
    <q-dialog v-model="dlgUpload" persistent>
      <q-card class="icard" style="width: 500px" square>
        <q-bar class="entry-caption">Upload dokumen - {{edit.vehicle_no}} - {{edit.descriptions}}
          <q-space />
          <q-btn v-close-popup dense glossy rounded icon="close" color="red-5" size="xs">
            <q-tooltip>Close</q-tooltip>
          </q-btn>
        ...
        <q-card-section class="q-pa-xs">
          <div class="row items-start q-col-gutter-sm q-mb-xs">
            <div class="col-12">
              <q-input v-model="doc.title" dense outlined square label="Judul" stack-label />
            </div>
          </div>
          <div class="row items-start q-col-gutter-sm q-mb-xs">
            <div class="col-12">
              <q-file v-model="doc.document_link" outlined dense label="Dokumen (Max 2 MB)" accept=".jpg,image/*"
                max-file-size="2056000" @rejected="onRejected" square stack-label>
                <template v-slot:prepend>
          ...
          <div class="row items-start q-col-gutter-sm q-mb-xs">
            <div class="col-12">
              <q-input v-model="doc.descriptions" dense outlined square label="Keterangan" type="textarea"
                input-style="height:60px" stack-label />
            </div>
          </div>
        </q-card-section>
        <q-card-section class="entry-caption q-pa-xs" align="right">
          <q-btn label="Upload" :loading="btn_loading" no-caps icon="fas fa-file-upload" @click="send_upload()" flat />
        </q-card-section>
      </q-card>
    </q-dialog>

    <q-dialog v-model="dlgPreview">
      <q-img :src="imglink" :ratio="4/3">
        <q-icon class="absolute all-pointer-events" size="xs" name="fas fa-times" color="red-10"
          style="top: 10px; left: 10px" @click="dlgPreview=false">
          <q-tooltip>
            Tutup
          </q-tooltip>
        </q-icon>
...
export default defineComponent({
  ...
  setup() {
    ...
    const col_document=ref([
      { name: "update_timestamp", align: "left", label: "Tanggal & Jam", field: "update_timestamp", sortable: true },
      { name: "document_link", align: "left", label: "Dokumen", field: "document_link"},
      { name: "descriptions", align: "left", label: "Catatan", field: "descriptions" },
    ]);
    ...
    async function document_event (primary = '') {
      if (selected.value.length > 0 || !(primary === '')) {
        ...
        try {
          ...
          doc.forEach(el=>{
            el.url_link = config.UrlLink() + '/master/operational/vehicle/download?sysid=' +el.sysid
    ...
    function preview_image(sysid=-1){
      let config = new Config()
      imglink.value = config.UrlLink() + '/master/operational/vehicle/download?sysid=' + sysid
      config = null
