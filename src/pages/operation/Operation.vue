<template>
  <q-page class="page-app">
    <q-card v-show="!stateform && (mode_data==='order')" square class="icard q-mb-sm">
      ...
      <q-card-section class="q-pa-sm">
        <div class="row items-center q-col-gutter-sm">
          ...
          <div class="col-xs-6 col-sm-3 col-md-2" v-show="state==='1'">
          ...
          <div class="col-xs-12 col-sm-3 col-md-3">
            <q-select v-model="vehicle_group" :options="main_types" outlined dense options-dense label="Grup unit"
              option-value="main_group" option-label="main_group" emit-value map-options fill-input stack-label square
              @update:model-value="loaddata()" />
          </div>
          <div v-show="((!stateform) && (state==='1'))" class="col-xs-12  col-sm-12 offset-md-1 col-md-2">
            <q-btn-toggle v-model="state" color="blue-grey-14" text-color="white" toggle-color="orange"
              toggle-text-color="primary" unelevated glossy no-caps spread :options="[
                {label: 'Semua', value: '1'},
                {label: 'Sedang Proses', value: '0'}
              ]" @click="loaddata()" />
          </div>
          <div v-show="((!stateform) && (state==='0'))" class="col-xs-12 offset-sm-2 col-sm-4 offset-md-5 col-md-2">
            <q-btn-toggle v-model="state" color="blue-grey-14" text-color="white" toggle-color="orange"
              toggle-text-color="primary" unelevated glossy no-caps spread :options="[
                {label: 'Semua', value: '1'},
                {label: 'Sedang Proses', value: '0'}
    ...
    <q-card v-show="!stateform && (mode_data==='order')" square class="icard">
      <q-bar class="entry-caption">
            ...
            <q-td v-for="col in props.cols" :key="col.name" :props="props">
              <div class="grid-data">
                ...
                <div v-else-if="col.name === 'work_order_no'">
                  <span :class="(props.row.work_status==='Closed') ? 'q-pa-xs bg-green-10 text-white' 
                    :(props.row.work_status==='Canceled') ? 'q-pa-xs bg-red-10 text-white'
                    :'q-pa-xs'">{{props.row.work_order_no}}</span>
                </div>
                ...
                <div v-else-if="col.name === 'expense'">
                  <div v-if="(props.row.standart_fleet_cost<props.row.expense)">
                    <span class="bg-red-10 text-white q-pa-xs"> {{ $formatnumber(props.row.expense) }} </span>
                  </div>
                  <div v-else>
                    <span class="q-pa-xs"> {{ $formatnumber(props.row.expense) }} </span>
                  </div>
                </div>
    ...
    <!-- Reservasi Order-->
    <q-card v-show="!stateform && (mode_data==='reservasi')" square class="icard q-mb-sm">
      <q-bar class="entry-caption">
        <span><strong>Periode</strong></span>
      </q-bar>
      <q-card-section class="q-pa-sm">
        <div class="row items-center q-col-gutter-sm">
          <div class="col-xs-12 col-sm-4 col-md-3">
            <q-select v-model="pool_code" :options="pools_all" outlined dense options-dense label="Pool kendaraan"
              option-value="pool_code" option-label="pool_name" emit-value map-options fill-input stack-label square
              @update:model-value="onRequestRev({pagination:pagination_rev,filter:filter_rev})" />
          </div>
        </div>
      </q-card-section>
    </q-card>

    <q-card v-show="((!stateform) && (mode_data==='reservasi'))" square class="icard">
      <q-bar class="entry-caption">
        <q-btn icon="refresh" color="green-10" size="sm" class="q-mr-sm" rounded dense @click="loaddata()" />
        <span><strong>Reservasi Order</strong></span>
        <q-space />
        <q-input v-model="filter_rev" dense outline debounce="300" label-color="white" borderless
      ...
      <q-table square :rows="data_rev" :columns="columns_rev" no-data-label="data kosong" :dense="$q.screen.md"
        no-results-label="data yang kamu cari tidak ditemukan" row-key="transid" :filter="filter_rev" separator="cell"
        selection="single" v-model:selected="selected_rev" v-model:pagination="pagination_rev" binary-state-sort
        @request="onRequestRev" :virtual-scroll="(mode_data==='reservasi')" :loading="loading_rev">
        <template v-slot:loading>
          <q-inner-loading showing>
            <q-spinner-ball size="75px" color="red-10" />
          </q-inner-loading>
            ...
            <q-td v-for="col in props.cols" :key="col.name" :props="props">
              <div class="grid-data">
                <div v-if="col.name === 'transid'">
                  <q-btn glossy rounded label="Buat Planning" dense no-caps class="bg-green-10 text-white"
          ...
          <div class="col-xs-12 col-sm-12 col-md-6">
            <div class="row items-start q-col-gutter-sm q-mb-sm">
              <div class="col-xs-4 col-sm-4 col-md-4">
                <q-input v-model="edit.work_order_no" dense outlined square label="No.Surat Muatan" stack-label
                  readonly />
              </div>
              <div class="col-xs-4 col-sm-4 col-md-4">
                <q-input v-model="edit.ref_date" dense outlined square type="date" label="Tg. Muatan" stack-label />
              </div>
              <div class="col-xs-4 col-sm-4 col-md-4">
                <q-select v-model="edit.project_id" dense outlined square label="Proyek" stack-label
                  :options="['BAK','BOX','DOKUMEN','GAS','KASBON','RCE','TRAILER','LAIN-LAIN']" options-dense />
              </div>
            </div>
            <div class="row items-start q-col-gutter-sm q-mb-sm">
              <div class="col-xs-4 col-sm-4 col-md-4">
                <q-input v-model="edit.order_no" dense outlined square label="No.Order" stack-label readonly>
                  <template v-slot:append>
                    <q-icon name="search" color="blue-grey-10" size="xs" @click="open_reservation()" />
                  </template>
                </q-input>
              </div>
              <div class="col-xs-4 col-sm-4 col-md-4">
            ...
            <div class="row items-start q-col-gutter-sm q-mb-sm" v-show="edit.partner_id==='2103'">
              <div class="col-12">
                <q-input v-model="edit.customer_name" dense outlined square label="Nama Konsumen" stack-label />
              </div>
            </div>
            <div class="row items-start q-col-gutter-sm q-mb-sm">
              <div class="col-xs-12 col-sm-6 col-md-6">
                <q-select v-model="edit.vehicle_group" :options="vehicle_types" outlined dense options-dense
                  ...
                  map-options fill-input stack-label square disable />
              </div>
              <div class="col-xs-6 col-sm-2 col-md-2">
                <q-input v-model="edit.eta" dense outlined square label="ETA" stack-label />
              </div>
              <div class="col-xs-6 col-sm-4 col-md-4">
            ...
            <div class="row items-start q-col-gutter-sm q-mb-sm">
              <div class="col-xs-6 col-sm-6 col-md-6">
                <q-input v-model="edit.warehouse" dense outlined square label="Pabrik/Gudang asal" stack-label />
              </div>
              <div class="col-xs-6 col-sm-6 col-md-6">
                <q-input v-model="edit.warehouse_destination" dense outlined square label="Pabrik/Gudang tujuan"
                  stack-label />
              </div>
            </div>
            <div class="row items-start q-col-gutter-sm q-mb-sm">
            ...
            <div class="row items-start q-col-gutter-sm q-mb-sm">
              <div class="col-xs-6 col-sm-3 col-md-3">
                <q-input v-model="edit.vehicle_no" dense outlined square label="No.Unit" stack-label readonly>
                  <template v-slot:append>
                    <q-icon name="search" color="blue-grey-10" size="xs" @click="open_vehicle()" />
                  </template>
                </q-input>
              </div>
              <div class="col-xs-6 col-sm-3 col-md-3">
              ...
              <div class="col-xs-12 col-sm-6 col-md-6">
                <q-input v-model="edit.driver_name" dense outlined square label="Pengemudi" stack-label readonly>
                  <template v-slot:append>
                    <q-icon name="search" color="blue-grey-10" size="xs" @click="open_driver()" />
                  </template>
                </q-input>
              </div>
            </div>
            <div class="row items-start q-col-gutter-sm q-mb-sm">
              <div class=" col-xs-3 col-sm-3 col-md-3">
                <q-field square outlined stack-label dense :model-value="edit.price" label="Tarif">
                  <template v-slot:control>
                    <vue-numeric v-model="edit.price" class="q-field__input text-right" separator="." :precision="0"
                      readonly />
                  </template>
                </q-field>
              </div>
              <div class=" col-xs-3 col-sm-3 col-md-3">
                <q-field square outlined stack-label dense :model-value="edit.minimal_tonase" label="Min.Tonase">
                  <template v-slot:control>
                    <vue-numeric v-model="edit.minimal_tonase" class="q-field__input text-right" separator="."
                      readonly />
                  </template>
                </q-field>
              </div>
              <div class=" col-xs-3 col-sm-3 col-md-3">
                <q-field square outlined stack-label dense :model-value="edit.real_tonase" label="Real Tonase">
                  <template v-slot:control>
                    <vue-numeric v-model="edit.real_tonase" class="q-field__input text-right" separator="."
                      :precision="0" @input="change_tonase()" />
                  </template>
                </q-field>
              </div>
              <div class=" col-xs-3 col-sm-3 col-md-3">
                <q-field square outlined stack-label dense :model-value="edit.tonase" label="Tonase">
                  <template v-slot:control>
                    <vue-numeric v-model="edit.tonase" class="q-field__input text-right" separator="." :precision="0"
            ...
            <div class="row items-start q-col-gutter-sm q-mb-sm">
              <div class=" col-xs-3 col-sm-3 col-md-3">
                <q-field square outlined stack-label dense :model-value="edit.net_price" label="Net Tarif">
                  <template v-slot:control>
                    <vue-numeric v-model="edit.net_price" class="q-field__input text-right" separator="." :precision="0"
                      readonly />
                  </template>
                </q-field>
              </div>
              <div class=" col-xs-3 col-sm-3 col-md-3">
                <q-field square outlined stack-label dense :model-value="edit.over_night" label="Biaya Inap">
                  <template v-slot:control>
                    <vue-numeric v-model="edit.over_night" class="q-field__input text-right" separator="." readonly />
                  </template>
                </q-field>
              </div>
              <div class=" col-xs-3 col-sm-3 col-md-3">
                <q-field square outlined stack-label dense :model-value="edit.other_bill" label="Lain-Lain">
                  <template v-slot:control>
                    <vue-numeric v-model="edit.other_bill" class="q-field__input text-right" separator="." readonly />
                  </template>
                </q-field>
              </div>
              <div class=" col-xs-3 col-sm-3 col-md-3">
                <q-field square outlined stack-label dense :model-value="edit.total" label="Total Tarif">
                  <template v-slot:control>
                    <vue-numeric v-model="edit.total" class="q-field__input text-right" separator="." :precision="0"
                      readonly />
                  </template>
                </q-field>
              </div>
            ...
            <div class="row items-start q-col-gutter-sm q-mb-sm">
              <div class="col-xs-12 col-sm-12 col-md-12">
                <q-input v-model="edit.order_instructions" dense outlined square label="Keterangan" stack-label
                  type="textarea" input-style="min-height:30px" />
              </div>
            </div>
          </div>
          <div class="col-xs-12 col-sm-12 col-md-6">
              ...
              <q-card-section class="q-pa-xs">
                <div class="row items-start q-col-gutter-xs q-mb-xs">
                  <div class="col-xs-6 col-sm-4 col-md-3">
                    <q-input v-model="cost.doc_number" dense outlined square label="No. Pengajuan" stack-label
                      readonly />
                  </div>
                  <div class="col-xs-6 offset-sm-4 col-sm-4 offset-md-6 col-md-3">
                    <q-input v-model="cost.ref_date" dense outlined square label="Tanggal" stack-label readonly />
                  </div>
                </div>
                <div class="row items-start q-col-gutter-xs q-mb-xs">
                  <div class="col-xs-6 col-sm-4 col-md-3">
                    <q-field square outlined stack-label dense :model-value="edit.standart_fleet_cost"
                      label="B.O Standart">
                      <template v-slot:control>
                        <vue-numeric v-model="edit.standart_fleet_cost" class="q-field__input text-right" separator="."
                          readonly />
                      </template>
                    </q-field>
                  </div>
                  <div class="col-xs-6 offset-sm-4 col-sm-4 offset-md-6 col-md-3">
                    <q-select v-model="cost.dp_method" dense outlined square label="Terima UJO" stack-label
                      options-dense :options="['-','DITERIMA SUPIR','TRANSFER']" />
                  </div>
                </div>
                <div class="row items-start q-col-gutter-xs q-mb-xs">
                  <div class="col-xs-6 col-sm-4 col-md-3">
                  ...
                  <div class="col-xs-6 offset-sm-4 col-sm-4 offset-md-6 col-md-3">
                    <q-field square outlined stack-label dense :model-value="cost.total" label="B.O Pengajuan">
                      <template v-slot:control>
                      ...
                      <div class="grid-data">
                        ...
                        <div v-else-if="col.name === 'expense_budget'">
                          <q-field square outlined stack-label dense :model-value="props.row.expense_budget">
                            <template v-slot:control>
                              <vue-numeric v-model="props.row.expense_budget" class="q-field__input text-right"
                                separator="." :disabled="(props.row.is_fix==='1')" @input="calculate_cost()" />
                            </template>
                          </q-field>
                        </div>
    ...
    <q-page-sticky expand position="top">
      <q-toolbar class="main-toolbar">
        ...
        <q-space />
        <q-btn-toggle v-model="mode_data" color="blue-grey-14" text-color="white" toggle-color="orange"
          toggle-text-color="primary" unelevated glossy no-caps spread :options="[
    ...
    <driver v-if="dlgDriver" :show="dlgDriver" :pool_code="edit.pool_code" @CloseDriver="getDriver" />
    <vehicle v-if="dlgVehicle" :show="dlgVehicle" :pool_code="edit.pool_code" @CloseVehicle="getVehicle" />
    <geofance v-if="dlgGeofance" :show="dlgGeofance" state="C" @CloseGeofance="getGeofance" />

    <q-dialog v-model="dlgReservation" persistent transition-show="scale" transition-hide="scale" full-width>
      <q-card class="icard" style="width: 800px; max-width: 80vw; max-height: 700px">
        <q-bar class="entry-caption">Reservasi Order
          <q-space />
          <q-btn icon="close" color="red-10" size="xs" glossy dense rounded v-close-popup />
        </q-bar>
        <q-table square :rows="data_rev" :columns="columns_rev" no-data-label="data kosong" :dense="$q.screen.md"
          no-results-label="data yang kamu cari tidak ditemukan" row-key="transid" :filter="filter_rev" separator="cell"
          selection="single" v-model:selected="selected_rev" v-model:pagination="pagination_rev" binary-state-sort
          class="fit" table-style="min-height:302px; max-height: 450px" @request="onRequestRev" virtual-scroll
          :loading="loading_rev">
          <template v-slot:loading>
            <q-inner-loading showing>
              <q-spinner-ball size="75px" color="red-10" />
              ...
              <q-td v-for="col in props.cols" :key="col.name" :props="props">
                <div class="grid-data">
                  <div v-if="col.name === 'order_no'">
                    <q-btn glossy rounded :label="props.row.order_no" dense no-caps class="bg-green-10 text-white"
    ...
    <q-dialog v-model="dlgCancel" persistent>
      <q-card class="icard" style="min-width:600px">
        <q-bar class="entry-caption">Pembatalan Kegiatan Operasi
          <q-space />
          <q-btn dense glossy rounded icon="close" color="red-10" size="xs" v-close-popup />
        </q-bar>
        <q-card-section class="q-gutter-sm">
          <div class="row items-start q-col-gutter-sm q-mb-sm">
            <div class="col-4">
            ...
            <div class="col-4">
              <q-input v-model="edit.order_no" dense outlined square label="No.Order" stack-label disable />
            </div>
            <div class="col-4">
              <q-select v-model="edit.work_order_type" dense outlined square label="Tipe Order" stack-label
                :options="['RITASE','TONASE']" options-dense disable />
            </div>
          </div>
          <div class="row items-start q-col-gutter-sm q-mb-sm">
            <div class="col-4">
              <q-input v-model="edit.work_order_no" dense outlined square label="Surat Muatan" stack-label disable />
            </div>
            <div class="col-4">
              <q-input v-model="edit.ref_date" dense outlined square label="Tgl.Muatan" stack-label type="date"
                disable />
            </div>
            <div class="col-4">
              <q-select v-model="edit.work_type" dense outlined square label="Tipe Pekerjaan" stack-label
                :options="['EXPORT','IMPORT','DOMESTIC','CARGO','POK']" options-dense disable />
            </div>
          </div>
          <div class="row items-start q-col-gutter-sm q-mb-sm">
            <div class="col-8">
              <q-input v-model="edit.partner_name" dense outlined square label="Nama Konsumen" stack-label
                placeholder="Klik tombol untuk mencari nama konsumen" disable />
            </div>
            <div class="col-4">
              <q-input v-model="edit.customer_no" dense outlined square label="Nomor order konsumen" stack-label
                disable />
          ...
          <div class="row items-start q-col-gutter-sm q-mb-sm">
            <div class="col-4">
              <q-input v-model="edit.vehicle_no" dense outlined square label="No. Unit" stack-label disable />
            </div>
            <div class="col-4">
              <q-input v-model="edit.police_no" dense outlined square label="Nomor Polisi" stack-label disable />
            </div>
            <div class="col-4">
              <q-input v-model="edit.driver_name" dense outlined square label="Pengemudi" stack-label disable />
            </div>
          </div>
          <div class="row items-start q-col-gutter-sm q-mb-sm">
            <div class="col-12">
              <q-select v-model="edit.reason" dense outlined square label="Alasan Pembatatalan" stack-label
                :options="['Unit tidak ada','Pengemudi tidak ada','Salah reservasi','Lain-Lain']" options-dense />
            </div>
          </div>
        </q-card-section>
        <q-card-section class="q-pa-sm entry-caption" align="right">
          <q-btn label="Pembatalan Kegiatan" no-caps dense glossy icon="cancel" @click="cancel_order()" />
        </q-card-section>
      </q-card>
    </q-dialog>
...
export default defineComponent({
  ...
  setup() {
    ...
    const columns_rev = ref([
      ...
      { name: 'order_no', align: 'left', label: 'Nomor Reservasi', field: 'order_no', sortable: true },
      { name: 'order_date', align: 'left', label: 'Tanggal', field: 'order_date', sortable: true },
      { name: 'pool_code', align: 'left', label: 'Pool', field: 'pool_code', sortable: true },
      { name: 'partner_name', align: 'left', label: 'Konsumen', field: 'partner_name', sortable: true },
      { name: 'vehicle_type', align: 'left', label: 'Tipe Unit', field: 'vehicle_type', sortable: true },
      ...
      { name: 'warehouse', align: 'left', label: 'Lok.Muatan', field: 'warehouse', sortable: true },
    ...
    async function lbo_event(primary =-1) {
      if (selected.value.length > 0 || !(primary === -1)) {
        ...
        $router.push({ path: '/operational/work-order/lbo', query: {sysid:primary,pageback:'operation'} });
      }
    }
    async function cost_event (primary = -1) {
      if (selected.value.length > 0 || !(primary === -1)) {
        ...
        $router.push({ path: '/operational/work-order/closed-lbo', query: { sysid: primary } });
      }
    }
    ...
    function calculate_price(){
      ...
      edit.value.total = edit.value.net_price + edit.value.other_bill + edit.value.over_night
    }
