  ...
  <q-page class="page-app">
    ...
    <q-card class="icard" square>
      ...
      <q-card-section class="q-gutter-sm">
        ...
        <div class="row items-start q-col-gutter-sm q-mb-sm">
          <div class="col-xs-6 col-sm-4 col-md-2">
            <q-input v-model="edit.ref_date" dense outlined square label="Tgl.Kegiatan" stack-label type="date" disable />
          </div>
          <div class="col-xs-6 col-sm-4 col-md-2">
            <q-input v-model="edit.cost_date" dense outlined square label="Tgl.Reg.Biaya" stack-label type="date" />
          </div>
        </div>
      </q-card-section>
            ...
            <q-td v-for="col in props.cols" :key="col.name" :props="props">
              <div class="grid-data">
                ...
                <div v-else-if="col.name === 'expense'">
                  <q-field square outlined stack-label dense :model-value="props.row.expense" borderless>
                ...
                <div v-else-if="col.name === 'is_invoice'">
                  <q-checkbox v-model="props.row.is_invoice" true-value="1" false-value="0" dense />
                </div>
                <div v-else-if="col.name === 'acuan'">
                  <q-select v-model="props.row.acuan" :options="['COST','FEE']" dense options-dense outlined square />
                </div>
                <div v-else-if="col.name === 'fee'">
                  <q-field square outlined stack-label dense :model-value="props.row.fee" borderless>
                    <template v-slot:control>
                      <vue-numeric v-model="props.row.fee" class="q-field__input text-right" separator="."
    ...
    <q-page-sticky expand position="top">
      <q-toolbar class="main-toolbar">
        ...
        <q-btn dense no-caps flat class="btn-toolbar q-mr-xs" icon="save" label="Simpan" @click="save_cost()">
          <q-tooltip content-class="tooltips-app" :loading="btn_loading">
...
export default defineComponent({
  ...
  setup() {
    ...
    const columns_cost = ref([
      ...
      { name: 'standar_budget', align: 'right', label: 'Standar', field: 'standar_budget' },
      { name: 'budget', align: 'right', label: 'Kasir', field: 'budget' },
      { name: 'expense', align: 'right', label: 'Realisasi', field: 'expense' },
      { name: 'is_invoice', align: 'center', label: 'Invoice', field: 'is_invoice' },
